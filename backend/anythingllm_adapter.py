...
def convert_deepseek_to_anythingllm_format(messages: List[Dict[str, str]], adapter: AnythingLLMAdapter) -> Dict[str, Any]:
    ...
    formatted_message = f"""Создай UI компонент используя готовые CSS классы из дизайн-систем.

ВАЖНО: 
- Используй ТОЛЬКО готовые классы, НЕ inline стили
- Оборачивай HTML в <VISUAL>...</VISUAL>
- Указывай ширину в <SIZE>ширина</SIZE>
